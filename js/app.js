 ...
 */
    ...
    (() => {
        ...
        if (menuLists) menuLists.forEach((list => {
            const links = list.querySelectorAll(".menu__link");
            links.forEach(((link, index) => {
                const span = link.querySelector("span");
                if (span) {
                    const indexNumber = index + 1;
                    span.textContent = indexNumber >= 10 ? indexNumber : indexNumber.toString().padStart(2, "0");
                }
            }));
        }));
        ...
        function moveDescr() {
            var wrapper = document.querySelector(".process__wrapper");
            if (wrapper && window.innerWidth < 767.98) {
                var items = wrapper.querySelectorAll(".process__item");
                items.forEach((function(item) {
                    var descr = item.querySelector(".process__descr");
                    if (descr) item.insertBefore(descr, item.firstChild);
                }));
            }
        }
        function resetDescr() {
            ...
            if (wrapper) {
                ...
                items.forEach((function(item) {
                    var descr = item.querySelector(".process__descr");
                    if (descr) {
                        var row = item.querySelector(".process__row");
                        if (row) row.insertBefore(descr, row.firstChild);
                    }
                }));
            }
        }
        ...
        function moveTariffs(wrapper) {
            if (wrapper && window.innerWidth < 991.98) {
                var items = wrapper.querySelectorAll(".tariffs__wrap");
                items.forEach((function(item) {
                    var descr = item.querySelector(".tariffs__list");
                    var tariffsBottom = item.querySelector(".tariffs__bottom");
                    if (tariffsBottom) tariffsBottom.insertBefore(descr, tariffsBottom.firstChild);
                }));
            }
        }
        function resetTariffs(wrapper) {
            if (wrapper) {
                var items = wrapper.querySelectorAll(".tariffs__wrap");
                items.forEach((function(item) {
                    var descr = item.querySelector(".tariffs__list");
                    if (descr) {
                        var row = item.querySelector(".tariffs__top");
                        if (row) row.insertBefore(descr, row.lastChild);
                    }
                }));
            }
        }
